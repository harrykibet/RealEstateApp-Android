package com.application.real_estate_app.feature_property.viewModels
...
import com.application.real_estate_app.feature_property.data.AddPropertyUiState
...
class AddPropertyViewModel @Inject constructor(
    ...
    private val _bathrooms = MutableStateFlow(0)
    val bathrooms: StateFlow<Int> = _bathrooms.asStateFlow()

    private val _areaSize = MutableStateFlow(0.0)
    ...
    private val _leaseTerms = MutableStateFlow("")
    val leaseTerms: StateFlow<String> = _leaseTerms.asStateFlow()

    private val _additionalFeatures = MutableStateFlow("")
    ...
    fun <T> updateField(field: AddPropertyField, value: T) {
        when (field) {
            ...
            AddPropertyField.SelectedImageUris -> {
                _uiState.update { it.copy(selectedImageUris = value as List<Uri>) }
            }

            AddPropertyField.SelectedVideoUris -> {
                _uiState.update { it.copy(selectedVideoUris = value as List<Uri>) }
            }
    ...
    fun saveProperty() {
        ...
        val property = Property(
            ...
            price = state.price,
            latitude = state.propertyLocation?.latitude ?: 0.0,
            longitude = state.propertyLocation?.longitude ?: 0.0,
            ...
            createdAt = Date(),
            viewsCount = 0,   //RESERVED, Not implemented yet
