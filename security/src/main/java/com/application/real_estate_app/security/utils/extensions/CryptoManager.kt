...
private const val RSA_ENCRYPTED_KEY_SIZE = 384  // 3072 bits = 384 bytes
...
class CryptoManager @Inject constructor(
    ...
    override suspend fun encrypt(bytes: ByteArray): Result<ByteArray> = withContext(Dispatchers.IO) {
        try {
    ...
    override suspend fun decrypt(bytes: ByteArray): Result<ByteArray> = withContext(Dispatchers.IO) {
        try {
    ...
    override suspend fun rsaEncrypt(data: ByteArray): Result<ByteArray> = withContext(Dispatchers.IO) {
        try {
    ...
    override suspend fun rsaDecrypt(data: ByteArray): Result<ByteArray> = withContext(Dispatchers.IO) {
        try {
    ...
    override suspend fun verifySignature(data: ByteArray, signature: ByteArray): Result<Boolean> = withContext(Dispatchers.IO) {
        try {
            Signature.getInstance(SIGNATURE_ALGORITHM).apply {
    ...
    private fun generateAesKey() {
        KeyGenerator.getInstance(KeyProperties.KEY_ALGORITHM_AES, ANDROID_KEYSTORE).apply {
            init(
                KeyGenParameterSpec.Builder(AES_KEY_ALIAS,
    ...
    override suspend fun hashWithSalt(data: String): Result<String> = withContext(Dispatchers.IO) {
        try {
            ...
            SecretKeyFactory.getInstance("PBKDF2WithHmacSHA256").generateSecret(keySpec).let { secret ->
                Result.Success(
    ...
    override suspend fun verifyHash(data: String, hash: String): Result<Boolean> = withContext(Dispatchers.IO) {
        try {
